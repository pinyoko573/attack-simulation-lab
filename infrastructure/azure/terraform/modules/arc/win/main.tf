...
data "azurerm_arc_machine" "arcs-win" {
  name                = "arcs-win"
  resource_group_name = var.rg.name
}

resource "azurerm_arc_machine_extension" "arcs-win-ext" {
  name           = "arcs-win-ext"
  location       = var.rg.location
  arc_machine_id = data.azurerm_arc_machine.arcs-win.id
...
resource "azurerm_monitor_data_collection_rule_association" "dcra-win" {
  name                    = "dcra-win"
  target_resource_id      = data.azurerm_arc_machine.arcs-win.id
